  ...
  <div>
    ...
    <div>
      ...
      <el-table :data="list" border stripe v-loading.box="loading">
        ...
        <el-table-column prop="type" :label="$t('data.type')" :formatter="tableFilter.formatPageType"></el-table-column>

        <el-table-column prop="activity_name" :label="$t('data.appPage')"></el-table-column>

        <el-table-column prop="user_types" :label="$t('data.userType')"></el-table-column>
...
<script>
import { menus, delMenu } from '@/assets/api/menuModule'
...
export default {
  ...
  methods: {
    requestList (searchForm) {
      ...
      menus(Object.assign(sendData, search)).then(res => {
        let data = res.data
